app.controller("material-ctrl", function ($scope, $http) {
    ...
    $scope.timKiem = function () {
        ...
        if (trangThai == "") {
            ...
            $http.get(`/rest/material/timKiem/${name}/${trangThai}`).then(resp => {
                $scope.items = resp.data;
            })
        } else {
            $http.get(`/rest/material/timKiem/${name}/${trangThai}`).then(resp => {
    ...
    $scope.genderStart = function (obj) {
        ...
        if (trangThai == "") {
            ...
            $http.get(`/rest/material/timKiem/${trangThai}`).then(resp => {
                $scope.items = resp.data;
            })
        }
	...
	$scope.create = function () {
		...
		$http.get(`/rest/material/${item.name}`).then(resp =>{
			if (resp.data == '') {
	...
	$scope.update = function () {
		...
		$http.put(`/rest/material/${item.id}`, item).then(resp => {
	...
	$scope.delete = function (item) {
		...
		if (t == false) {
			...
			//alert("có xóa");
			/*$http.delete(`/rest/material/${item.id}`).then(resp => {
				var index = $scope.items.findIndex(p => p.id == item.id);
			...
			$http.put(`/rest/material/delete/${item.id}`, item).then(resp => {
				var index = $scope.items.findIndex(p => p.id == item.id);
				$scope.items[index] = item;
				alert("xóa thành công");
				$scope.initialize();
	...
	$scope.check = function (name){
		$http.get(`/rest/material/${name}`).then(resp =>{
			if (resp.data == '') {
